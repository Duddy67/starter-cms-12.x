...
class Document extends Model
{
	...
	// public => storage/app/public, local => storage/app/private
	$name = ($public) ? 'public' : 'local';

        // Note: Empty string will store the file at the root of directory.
	Storage::disk($name)->putFileAs('', $file, $this->disk_name);

	if (preg_match('#^image\/#', $this->content_type)) {
	    // Note: Empty string leaves out the file name from the path. 
	    $imagePath = Storage::disk($name)->path('');
